parasails.registerPage('black-friday-last-chance', {
  //  ╦╔╗╔╦╔╦╗╦╔═╗╦    ╔═╗╔╦╗╔═╗╔╦╗╔═╗
  //  ║║║║║ ║ ║╠═╣║    ╚═╗ ║ ╠═╣ ║ ║╣
  //  ╩╝╚╝╩ ╩ ╩╩ ╩╩═╝  ╚═╝ ╩ ╩ ╩ ╩ ╚═╝
  props: {
...
// Add an instance of the card Element into the `card-element` <div>.
  ...
  computed: {
    dynamicAddress () {
      ...
      let fields = {};
      country[0].fields.forEach((field) => fields = {...fields, ...field});
      let dynamic = {};
      fields.locality.forEach((field) => dynamic = {...dynamic, ...field});
      return dynamic
    },
    dateInMilliseconds() {
  ...
  methods: {
    ...
    //     return;
    //   } else {
    //     this.syncingStates = true;
    //     await fetch(`https://geodata.solutions/api/api.php?type=getStates&countryId=${this.formData.country}&addClasses=order-alpha`)
    //       .then((response) => {console.log(response.json()); this.states = result})
    //       .catch(() => {
    //         document.getElementById('stateInput')
    //           .replaceWith('<input type="text" name="state"  class="mt-2" v-model.trim="formData.state"/>');
    //       });
    //     this.syncingStates = false;
    //   }
    // },
  }
});
