module.exports = {
  ...
  friendlyName: 'Send plain text template email',


  description: 'Send a plain text email using a template.',
  ...
  fn: async function(inputs) {
    ...
    if (dontActuallySend) {
      ...
      var deferred = Mailgun.sendPlaintextEmail({
        apiKey: sails.config.custom.mailgunSecret,
        domain: sails.config.custom.mailgunDomain,
        toEmail: inputs.to,
        subject: inputs.subject,
        message: htmlToText.fromString(htmlEmailContents, {
          ignoreImage: true,
