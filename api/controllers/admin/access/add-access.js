module.exports = {
  ...
  fn: async function (inputs) {
    ...
    if (access && access === 'free') {
      let userAccess = await UserSiteLinks.updateOrCreateAndFetch(
        {user_id: userData.id, site_id: 2},
        {user_id: userData.id, site_id: 2, usertype_id: 5, expiry: new Date(Date.now() + inputs.days * 24 * 60 * 60 * 1000)}
        )
