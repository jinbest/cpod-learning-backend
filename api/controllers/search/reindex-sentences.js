module.exports = {
  ...
  fn: async function (inputs) {
    ...
    let dialogues = await ContentDialogues.find().sort('id DESC').limit(5000).populate('v3_id');

    let expansions = await ContentExpansions.find().sort('id DESC').limit(5000).populate('v3_id');
