module.exports = {
  ...
  fn: async function (inputs) {
    let trial = false; let promo = true; let plan = 'premium'; let promoCode = 'LABORDAY828'; let nonRecurring = false;

    let period = inputs.quarterly ? 'quarterly' : 'monthly' // inputs.annual ? 'annually' : 'quarterly';

    let validPromos = await PromoCodes.find({promotion_code: promoCode, product_id: {in: [140, 2, 18, 142, 13, 14]}, expiry_date: {'>': new Date()}})

    sails.log.info(validPromos)
    ...
    // Respond with view.
    return this.res.view('pages/promotions/labor-day/promo',{
