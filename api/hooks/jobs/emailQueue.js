...
// OK.
  ...
  emailTriggerQueue.process('ScheduleInactivityEmailsTesting', 1, async function (job, done) {
    ...
    let spamList = (await EmailLogs.find().select('user_id')).map(item => item.user_id);

    let userData = await sails.models['user'].find({email: {in: liveUsers}});

    sails.hooks.bugsnag.notify(`Potential List - ${userData.length}`);

    userData = await sails.models['user'].find({email: {in: liveUsers}, id: {nin: spamList}});

    sails.hooks.bugsnag.notify(`Clean List - ${userData.length}`);
  ...
  emailTriggerQueue.removeRepeatable('ScheduleInactivityEmailsTesting', {group: 'asia'}, {repeat: {cron: '*/9 * * * *'}});
  emailTriggerQueue.removeRepeatable('ScheduleInactivityEmailsTesting', {group: 'europe'}, {repeat: {cron: '*/10 * * * *'}});
  emailTriggerQueue.removeRepeatable('ScheduleInactivityEmailsTesting', {group: 'testing'}, {repeat: {cron: '*/11 * * * *'}});
